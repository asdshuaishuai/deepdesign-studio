...
| 返回 | 底部「◀ 返回」或 Esc = playStack 回退上一页；起始页再按/Esc 退出演示。**强制返回**：形似返回的元素（id 词边界 back/prev/return，或文案 返回/←/‹ 等）即使未接 flow/interact 也挂热区并在点击时执行返回（状态栏标「强制」）——演示不因漏接而卡死；`background` 类 id 不会误判 |
| 切页动画 | 演示内导航带方向滑入过渡（前进左滑 / 返回右滑，仅 play 模式生效） |
| 标注（批注） | ✏ 开关：全屏手绘墨迹（中点贝塞尔平滑、橙色）；标注中热区点击暂停，Esc/再点退出。**持久化**：批注以 `<!-- deepdesign:ink … -->` 注释块随 `.mbt.md` 存进 DDP（引擎 canonical 会丢未知注释——moonviz#20，前端注入层在每次引擎回写后重新合并）；退出演示自动注入并置脏，⌘S 落盘；重开 DDP 进演示自动重放（视口归一化坐标，尺寸无关）。🧹 清除当前墨迹后保存即移除 |
| 撤销/重做 | 演示中禁用（⌘Z/⇧⌘Z 不响应） |
